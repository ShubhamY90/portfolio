...
function createTextTexture(gl, text, font = "bold 30px monospace", color = "black") {
  ...
  const texture = new Texture(gl, { generateMipmaps: false }); // ⚡ No mipmaps
...
class Media {
  ...
  createShader() {
    const texture = new Texture(this.gl, { generateMipmaps: false }); // ⚡ Disable mipmaps for perf
    const img = new Image();
  ...
  update(scroll, direction) {
    ...
    this.program.uniforms.uTime.value += 0.03; // ⚡ lower update freq
...
class App {
  ...
  createRenderer() {
    this.renderer = new Renderer({
      alpha: true,
      antialias: false, // ⚡ disable heavy AA
  ...
  createMedias() {
    const { items, bend, textColor, borderRadius, font } = this.opts;
  ...
  onTouchUp() {
    this.isDown = false;
    this.onCheck();
    setTimeout(() => (this.isAutoScrolling = true), 3000);
  }
  ...
  update() {
    if (this.isAutoScrolling && !this.isDown) this.scroll.target += this.autoScrollSpeed;
  ...
  destroy() {
    ...
    ["resize", "wheel", "mousedown", "mousemove", "mouseup", "touchstart", "touchmove", "touchend"].forEach((e) =>
      window.removeEventListener(e, this[`bound${e[0].toUpperCase() + e.slice(1)}`])
    );
    if (this.renderer?.gl?.canvas?.parentNode)
      this.renderer.gl.canvas.parentNode.removeChild(this.renderer.gl.canvas);
    this.medias?.forEach((m) => (m.program = null));
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      if (!app.viewport || !app.medias) return;
      const rect = containerRef.current.getBoundingClientRect();
      const x = ((e.clientX - rect.left) / rect.width) * 2 - 1;
      let closest = null,
        min = Infinity;
      app.medias.forEach((m, i) => {
        const dx = (m.plane.position.x / app.viewport.width) * 2;
      ...
      if (closest !== null && min < 0.8) onSelect(items[closest % items.length]);
    };
